  ...
  const buscar = useCallback(
    debounce(async (q: any) => {
      ...
      try {
        ...
        setError('No se pudo conectar. Revisa tu conexiÃƒÂ³n.')
        setResultados([])
      } finally {
        setCargando(false)
      }
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-modal border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-700">
                ...
                <input
                  ...
                  placeholder="Buscar clientes, telÃƒÂ©fonos, correos, notasÃ¢â‚¬Â¦"
              ...
              <div className="max-h-[60vh] overflow-y-auto" role="listbox">
                ...
                {!error && !cargando && termino.length >= 2 && !tieneResultados && (
                  <div className="px-4 py-8 text-center">
                    <p className="text-slate-500 dark:text-slate-400 text-sm">
                      No encontrÃƒÂ© nada con &quot;{termino}&quot;.
                    </p>
                    <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">
                      Revisa cÃƒÂ³mo lo escribiste o crea un cliente nuevo.
                    </p>
                    <a
                      href="/clientes/nuevo"
                      onClick={onCerrar}
                ...
                {!termino && recientes.length > 0 && (
                  <div className="px-4 py-3">
                    <p className="text-xs text-slate-400 mb-2 font-medium uppercase tracking-wide">BÃƒÂºsquedas recientes</p>
                    {recientes.map(r => (
                      <button
                        key={r}
                ...
                <div className="px-4 py-2 border-t border-slate-100 dark:border-slate-700 flex items-center gap-4 text-xs text-slate-400">
                  <span><kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-700 rounded">Ã¢â€ â€˜Ã¢â€ â€œ</kbd> navegar</span>
                  <span><kbd className="px-1 py-0.5 bg-slate-100 dark:bg-slate-700 rounded">Ã¢â€ Âµ</kbd> abrir</span>
