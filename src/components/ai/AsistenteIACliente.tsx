...
const FUNCIONES: { key: FuncionIA; label: string; icon: React.ComponentType<{ size?: number }>; descripcion: string }[] = [
  ...
  { key: 'temperatura', label: 'Temperatura', icon: Thermometer, descripcion: 'Clasificar qué tan cerca está de comprar' },
  { key: 'proxima_accion', label: 'Próxima acción', icon: CheckSquare, descripcion: 'Sugerir el siguiente paso concreto con fecha' },
  { key: 'resumen', label: 'Resumen', icon: FileText, descripcion: 'Resumen de 3–5 líneas de todo el expediente' },
  { key: 'objecion', label: 'Manejar objeción', icon: Shield, descripcion: 'Respuesta concreta para vencer la objeción del cliente' },
]
...
export function AsistenteIACliente({ clienteId, clienteNombre }: Props) {
  ...
  const ejecutar = async (fn: FuncionIA) => {
    ...
    try {
      ...
      setResultado('Error de conexión. Verifica tu internet e intenta de nuevo.')
    } finally {
      setCargando(false)
    }
  }
  ...
  const copiar = async () => {
    ...
    agregar({ tipo: 'exito', titulo: 'Copiado ✓', mensaje: '' })
  }

  const guardarComoAccion = async () => {
    if (!resultado || funcion !== 'proxima_accion') return
    // Extraer la primera línea como acción
    const lineas = resultado.split('\n').filter(l => l.trim())
    const accion = lineas[0]?.replace(/^[*-•]\s*/, '').substring(0, 200) || resultado.substring(0, 200)
    ...
    try {
      ...
      if ((await res.json()).ok) {
        ...
        agregar({ tipo: 'exito', titulo: 'Próxima acción guardada ✓', mensaje: '' })
      }
    } catch {
          ...
          {cargando ? (
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm" style={{ color: 'var(--text-secundario)' }}>
                Analizando expediente de {clienteNombre}…
              </span>
            </div>
          ) : (
            <>
              ...
              {accionGuardada && (
                <p className="text-xs text-center" style={{ color: 'var(--color-marca)' }}>
                  ✓ Próxima acción guardada en el expediente
                </p>
              )}
