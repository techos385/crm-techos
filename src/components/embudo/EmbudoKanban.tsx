  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-2 space-y-2 overflow-y-auto max-h-[60vh]">
        ...
        {clientes.length === 0 && (
          <p className="text-xs text-center py-4 opacity-40">Arrastra aquí</p>
        )}
      </div>
    </div>
  )
...
export function EmbudoKanban() {
  ...
  const onDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    try {
      ...
      agregar({ tipo: 'exito', titulo: `Movido a ${nuevaEtapa} ✓`, mensaje: '' })
    } catch {
  ...
  const ETAPAS_ACTIVAS = ETAPAS_EMBUDO.filter(e =>
    !['Proyecto Ganado', 'Perdido', 'Precio alto', 'Canceló proyecto', 'Eligió a la competencia', 'Sin presupuesto', 'No respondió'].includes(e)
  )
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="card px-4 py-2.5 flex items-center gap-2 text-sm">
          ...
          <span style={{ color: 'var(--text-secundario)' }}>·</span>
