...
// "Hoy te toca" + acciones vencidas + leads fríos + recordatorios
...
export function SeguimientoContenido() {
  ...
  const completarRecordatorio = async (id: string) => {
    ...
    cargar()
    agregar({ tipo: 'exito', titulo: 'Recordatorio completado ✓', mensaje: '' })
  }
  ...
  const TABS = [
    ...
    { key: 'frios', label: 'Leads fríos', count: datos.leadsFrios.length, icon: Snowflake, color: 'text-blue-400' },
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="card p-4 text-center">
          ...
          <p className="text-xs mt-1" style={{ color: 'var(--text-secundario)' }}>Leads fríos</p>
        </div>
        <div className="card p-4 text-center">
      ...
      {datos.sinAccion > 0 && (
        <div className="flex items-center gap-3 p-3 rounded-xl text-sm bg-orange-500/10 text-orange-600">
          ...
          <span><strong>{datos.sinAccion} clientes activos</strong> no tienen próxima acción — ponles una para que no se enfríen.</span>
        </div>
      )}
        ...
        <div className="space-y-2">
          {datos.hoyToca.length === 0 ? (
            <div className="card p-8 text-center">
              ...
              <p className="font-semibold">¡Hoy no tienes pendientes! 🎉</p>
      ...
      {tab === 'vencidos' && (
        <div className="space-y-2">
          {datos.vencidos.length === 0 ? (
            <EmptyState mensaje="Sin acciones vencidas" sub="¡Vas al día!" />
          ) : (
      ...
      {tab === 'frios' && (
        <div className="space-y-2">
          {datos.leadsFrios.length === 0 ? (
            <EmptyState mensaje="Sin leads fríos" sub="Todos tus prospectos están activos." />
          ) : (
      ...
      {tab === 'recordatorios' && (
        <div className="space-y-2">
          {datos.recordatorios.length === 0 ? (
            <EmptyState mensaje="Sin recordatorios" sub="Cuando agregues recordatorios a tus clientes aparecerán aquí." />
          ) : (
    ...
    <div className={cn('card p-4 flex items-center justify-between gap-3', vencido && 'border-red-500/30')}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {c.esNuevo && c.horasDesdeCreacion && c.horasDesdeCreacion > 24 && (
            <span className="text-xs text-red-500 bg-red-500/10 px-2 py-0.5 rounded-full flex items-center gap-1">
              <AlertTriangle size={10} /> Lead frío por demora
            </span>
          )}
        </div>
        <p className="text-sm mt-1 truncate" style={{ color: 'var(--text-secundario)' }}>
          ...
          {c.proximaAccionFecha && ` · ${formatearFechaCorta(c.proximaAccionFecha)}`}
        </p>
        {c.valorEstimado && (
