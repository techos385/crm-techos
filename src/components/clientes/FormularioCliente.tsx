...
export function FormularioCliente({ clienteInicial, onCerrar, onGuardado }: Props) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      agregar({ tipo: 'error', titulo: 'Sin conexión', mensaje: 'Revisa tu internet e intenta de nuevo.' })
    } finally {
      setGuardando(false)
    }
  }
      ...
      <div className="relative w-full sm:max-w-lg max-h-[95vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl"
        ...
        {duplicado && (
          <div className="mx-5 mt-4 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/30">
            <p className="font-medium text-yellow-600 dark:text-yellow-400">
              ⚠️ Ya tienes a {duplicado.nombre} con ese teléfono o correo.
            </p>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1.5">WhatsApp / Teléfono</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1.5">Empresa / Institución</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">Zona / Ubicación</label>
              <div className="relative">
                <MapPin size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-secundario)' }} />
                <input
                  type="text"
                  placeholder="Ecatepec, Estado de México"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1.5">¿De dónde llegó?</label>
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">
                Temperatura
                <span className="ml-1 text-xs font-normal" style={{ color: 'var(--text-secundario)' }}>¿Qué tan cerca está de comprar?</span>
              </label>
              <select value={form.temperatura} onChange={(e) => set('temperatura', e.target.value)} className="campo w-full">
                <option value="CALIENTE">🔥 Caliente — quiere hoy</option>
                <option value="TIBIO">🟡 Tibio — está viendo</option>
                <option value="FRIO">🔵 Frío — a futuro</option>
              </select>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">
              Objeción principal
              <span className="ml-1 text-xs font-normal" style={{ color: 'var(--text-secundario)' }}>¿Qué lo frena para comprar?</span>
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">Para cuándo</label>
              <input
                type="datetime-local"
          ...
          <div>
            ...
            <textarea
              placeholder="Qué sabes de este cliente, su proyecto, sus necesidades…"
              value={form.notas}
