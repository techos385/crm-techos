...
export function ExpedienteCliente({ id }: Props) {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      agregar({ tipo: 'error', titulo: 'Error al cargar', mensaje: 'Revisa tu conexiÃƒÂ³n' })
    } finally {
      setCargando(false)
    }
  }
  ...
  const guardarCambios = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setEditando(false)
        cargar()
        agregar({ tipo: 'exito', titulo: 'Guardado Ã¢Å“â€œ', mensaje: '' })
      }
    } catch {
  ...
  const agregarNota = async () => {
    ...
    try {
      ...
      if ((await res.json()).ok) {
        setNuevaNota('')
        cargar()
        agregar({ tipo: 'exito', titulo: 'Nota agregada Ã¢Å“â€œ', mensaje: '' })
      }
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      {celebrar && <CelebracionGanado nombre={String(c.nombre ?? '')} valor={c.valorEstimado as number} />}
          ...
          {editando ? (
            <>
              ...
              <button onClick={guardarCambios} disabled={guardando} className="btn-primario py-1.5 px-3 text-sm flex items-center gap-1.5">
                <Save size={14} />
                {guardando ? 'GuardandoÃ¢â‚¬Â¦' : 'Guardar'}
              </button>
            </>
          ) : (
      ...
      <div className="card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">{String(c.nombre ?? '')}</h1>
              {c.empresa && <p className="text-sm mt-0.5" style={{ color: 'var(--text-secundario)' }}>{String(c.empresa ?? '')}</p>}
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                <span className={cn('badge', c.estadoCartera === 'GANADO' ? 'text-green-600' : c.estadoCartera === 'PERDIDO' ? 'text-gray-600' : '')}>
                  {c.estadoCartera === 'ACTIVO' ? 'Ã¢â€”Â Activo' : c.estadoCartera === 'GANADO' ? 'Ã¢Å“â€¦ Completado' : c.estadoCartera === 'PERDIDO' ? 'Ã¢Å“â€” Perdido' : 'Ã°Å¸â€œÂ Archivado'}
                </span>
        ...
        <div className="mt-4 space-y-2">
          ...
          {diasSinContacto !== null && diasSinContacto > 7 && (
            <div className="flex items-center gap-2 text-sm text-red-500 bg-red-500/10 rounded-xl p-3">
              ...
              <span>Llevas <strong>{diasSinContacto} dÃƒÂ­as</strong> sin contactarlo Ã¢â‚¬â€ ponlo al frente hoy</span>
            </div>
          )}

          {/* PrÃƒÂ³xima acciÃƒÂ³n vencida */}
          {c.proximaAccionFecha && new Date(c.proximaAccionFecha as string) < new Date() && (
            <div className="flex items-center gap-2 text-sm text-orange-500 bg-orange-500/10 rounded-xl p-3">
              <Clock size={14} />
              <span>AcciÃƒÂ³n vencida: <strong>{String()}</strong> Ã¢â‚¬â€ {formatearFechaCorta(c.proximaAccionFecha as string)}</span>
            </div>
          )}
          ...
          {c.objecionPrincipal && (
            <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-secundario)' }}>
              <Target size={14} />
              <span>ObjeciÃƒÂ³n: <strong>{String()}</strong></span>
            </div>
          )}
          ...
          {c.proximaAccion && !(c.proximaAccionFecha && new Date(c.proximaAccionFecha as string) < new Date()) && (
            <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-secundario)' }}>
              <CheckCircle size={14} />
              <span>PrÃƒÂ³xima acciÃƒÂ³n: <strong>{String()}</strong>
                {c.proximaAccionFecha && ` Ã¢â‚¬â€ ${formatearFechaCorta(c.proximaAccionFecha as string)}`}
              </span>
            </div>
          )}
        </div>
        ...
        {c.estadoCartera === 'ACTIVO' && (
          <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t" style={{ borderColor: 'var(--border)' }}>
            <button
              onClick={() => { if (confirm('Ã‚Â¿Marcar como GANADO? Se moverÃƒÂ¡ a Clientes completados.')) cambiarEstado('GANADO') }}
              className="btn-secundario flex items-center gap-1.5 text-sm text-green-600 border-green-600/30"
            >
              <Trophy size={14} /> Marcar como ganado Ã°Å¸Å½â€°
            </button>
            <button
              onClick={() => {
                const motivo = prompt('Ã‚Â¿Por quÃƒÂ© se perdiÃƒÂ³? (precio / competencia / sin presupuesto / no respondiÃƒÂ³ / otro)')
            ...
            <button
              onClick={() => { if (confirm(`Ã‚Â¿Archivar a ${c.nombre}? PodrÃƒÂ¡s restaurarlo cuando quieras.`)) cambiarEstado('ARCHIVADO') }}
              className="btn-secundario flex items-center gap-1.5 text-sm"
            >
        ...
        <div className="space-y-4">
          <div className="card p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>TelÃƒÂ©fono / WhatsApp</label>
                {String()} onChange={(e) => set('telefono', e.target.value)} />
                  : <p className="font-medium">{c.telefono as string || 'Ã¢â‚¬â€'}</p>
                }
              </div>
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>Correo</label>
                {String()} onChange={(e) => set('correo', e.target.value)} />
                  : <p className="font-medium">{c.correo as string || 'Ã¢â‚¬â€'}</p>
                }
              </div>
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>Empresa</label>
                {String()} onChange={(e) => set('empresa', e.target.value)} />
                  : <p className="font-medium">{String(c.empresa ?? '') || 'Ã¢â‚¬â€'}</p>
                }
              </div>
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>Zona / UbicaciÃƒÂ³n</label>
                {String()} onChange={(e) => set('zonaUbicacion', e.target.value)} />
                  : <p className="font-medium">{c.zonaUbicacion as string || 'Ã¢â‚¬â€'}</p>
                }
              </div>
            </div>
          </div>
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {String()} onChange={(e) => set('tipoObra', e.target.value)} />
                  : <p className="font-medium">{c.tipoObra as string || 'Ã¢â‚¬â€'}</p>
                }
              </div>
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>Medidas del proyecto</label>
                {String()} onChange={(e) => set('medidasProyecto', e.target.value)} />
                  : <p className="font-medium">{c.medidasProyecto as string || 'Ã¢â‚¬â€'}</p>
                }
              </div>
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>Valor estimado (MXN)</label>
                {String()} onChange={(e) => set('valorEstimado', e.target.value)} />
                  : <p className="font-medium">{c.valorEstimado ? `$${(c.valorEstimado as number).toLocaleString('es-MX')}` : 'Ã¢â‚¬â€'}</p>
                }
              </div>
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secundario)' }}>ObjeciÃƒÂ³n principal</label>
                {String()} onChange={(e) => set('objecionPrincipal', e.target.value)} />
                  : <p className="font-medium">{c.objecionPrincipal as string || 'Ã¢â‚¬â€'}</p>
                }
              </div>
            </div>
          </div>
          ...
          <div className="card p-5">
            ...
            {!editando && (
              <div className="mt-4 flex gap-2">
                <textarea
                  placeholder="Agregar nota de interacciÃƒÂ³nÃ¢â‚¬Â¦"
                  value={nuevaNota}
          ...
          <div className="card p-4">
            ...
            >
              ...
              <span className="text-xs" style={{ color: 'var(--text-secundario)' }}>{mostrarIA ? 'Ã¢â€“Â² Cerrar' : 'Ã¢â€“Â¼ Abrir'}</span>
            </button>
            {mostrarIA && <AsistenteIACliente clienteId={id} clienteNombre={String(c.nombre ?? '')} />}
          </div>
        </div>
      )}
      ...
      {tab === 'timeline' && (
        <div className="card p-5 space-y-4">
          ...
          {timeline.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--text-secundario)' }}>Sin historial aÃƒÂºn</p>
          ) : (
                ...
                <div key={i} className="flex gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: 'var(--text-secundario)' }}>
                      ...
                      {evento.autor && ` Ã¢â‚¬â€ ${evento.autor}`}
                    </p>
                  </div>
                </div>
              ))}
                ...
                <div key={pago.id} className="card p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs" style={{ color: 'var(--text-secundario)' }}>
                      {pago.metodo} Ã‚Â· {pago.concepto ?? 'Sin concepto'}
                      {pago.fechaPago && ` Ã‚Â· ${formatearFechaCorta(pago.fechaPago)}`}
                    </p>
                  </div>
                  <span className={cn('badge text-xs', pago.estatus === 'PAGADO' ? 'text-green-600' : pago.estatus === 'VENCIDO' ? 'text-red-600' : '')}>
                    {pago.estatus === 'PAGADO' ? 'Ã¢Å“â€¦ Pagado' : pago.estatus === 'VENCIDO' ? 'Ã¢Å¡Â Ã¯Â¸Â Vencido' : 'Ã¢ÂÂ³ Pendiente'}
                  </span>
                </div>
              ))}
            </div>
              ...
              {archivos.map((a) => (
                <div key={a.id} className="card p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs" style={{ color: 'var(--text-secundario)' }}>
                      {a.etiqueta} Ã‚Â· {formatearFechaCorta(a.creadoEn)} Ã‚Â· por {a.subidoPor.nombre}
                    </p>
                  </div>
                  {a.tieneArchivo && (
...
function SubidaArchivos({ clienteId, onSubido }: { clienteId: string; onSubido: () => void }) {
  ...
  const subir = async (archivo: File) => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.ok) {
        agregar({ tipo: 'exito', titulo: 'Archivo guardado Ã¢Å“â€œ', mensaje: archivo.name })
        onSubido()
      } else {
        agregar({ tipo: 'error', titulo: 'Error al subir', mensaje: data.mensaje })
      }
    } catch {
      agregar({ tipo: 'error', titulo: 'Error de conexiÃƒÂ³n', mensaje: 'No se pudo subir el archivo' })
    } finally {
      setSubiendo(false)
      setProgreso(0)
    }
    ...
    <div className="card p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <select value={etiqueta} onChange={(e) => setEtiqueta(e.target.value)} className="campo text-sm py-1.5">
          ...
          <option value="CotizaciÃƒÂ³n">CotizaciÃƒÂ³n</option>
          <option value="IdentificaciÃƒÂ³n">IdentificaciÃƒÂ³n</option>
      ...
      <label className={cn(
        ...
        <p className="text-sm font-medium">{subiendo ? 'SubiendoÃ¢â‚¬Â¦' : 'Toca o arrastra un archivo'}</p>
        <p className="text-xs mt-1" style={{ color: 'var(--text-secundario)' }}>PDF, JPG, PNG Ã¢â‚¬â€ mÃƒÂ¡x. 5MB</p>
        {!subiendo && (
